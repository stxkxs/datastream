...
public class WikimediaKafkaConf {
  ...
  public NewTopic streamEventsTopic(KafkaConf conf) {
    return TopicBuilder
      .name(KafkaTopic.WIKIMEDIA_STREAMS_EVENT.value())
  ...
  @Bean
  public NewTopic testEventsTopic(KafkaConf conf) {
    return TopicBuilder
      .name(KafkaTopic.WIKIMEDIA_TEST_EVENT.value())
      .partitions(conf.admin().properties().partitions())
      .replicas(conf.admin().properties().replicas())
      .build();
  }
  ...
  private Map<String, Object> kafkaAuthConf(KafkaConf conf) {
    var kafkaConf = new java.util.HashMap<>(Map.of(
